...
#def data_augmentation(reshape_inputs, 32, 32):
#   flip_image = tf.map_fn(lambda)
...
with tf.name_scope('conv1') as scope:
    kernel = _variable_with_weight_decay('weights_1',
                                         ...
                                         stddev=5e-2,
                                         wd=0.0)
    conv = tf.nn.conv2d(reshape_inputs, kernel, [1, 1, 1, 1], padding='SAME')
...
train_c100_exp0 = {'epoch':[], 'error':[], 'accuracy':[], 'time':[]}
valid_c100_exp0 = {'epoch':[], 'error':[], 'accuracy':[]}
...
with tf.Session() as sess:
    ...
    for e in range(50):
        ...
        train_c100_exp0['epoch'].append(e+1)
        train_c100_exp0['error'].append(running_error)
        train_c100_exp0['accuracy'].append(running_accuracy)
        train_c100_exp0['time'].append(epoch_time)
        ...
        if (e + 1) % 1 == 0:
            ...
            valid_c100_exp0['epoch'].append(e+1)
            valid_c100_exp0['error'].append(valid_error)
            valid_c100_exp0['accuracy'].append(valid_accuracy)
...
ax1.plot(np.array(train_c100_exp0['error']), label='baseline')
ax1.plot(valid_c100_exp0['epoch'], valid_c100_exp0['error'], label='baseline')

ax2.plot(np.array(train_c100_exp0['accuracy']), label='baseline')
ax2.plot(valid_c100_exp0['epoch'], valid_c100_exp0['accuracy'], label='baseline')
...
plt.show()
fig1.tight_layout()
fig1.savefig('c100_0.png', dpi=200)
